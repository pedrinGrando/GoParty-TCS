...
export default function Notifications() {
    ...
    const fetchUserInvites = async (): Promise<InviteDTO[]> => {
        try {
            const response = await fetch(`http://localhost:8081/api/invites/user/${user.id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
                           ...
                           <div>
                            ...
                            <div className="flex">
                                ...
                                <div className="ms-3 text-sm font-normal">
                                    <span className="mb-1 text-sm font-semibold text-gray-900 dark:text-white">{invite.inviteDate}</span>
                                    <div className="mb-2 text-sm font-normal"></div> 
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <a href="#" className="inline-flex justify-center w-full px-2 py-1.5 text-xs font-medium text-center text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">Aceitar</a>
                                        </div>
                                        <div>
                                            <a href="#" className="inline-flex justify-center w-full px-2 py-1.5 text-xs font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:bg-gray-600 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-700 dark:focus:ring-gray-700">Rejeitar</a> 
                                        </div>
                                    </div>    
                                </div>
