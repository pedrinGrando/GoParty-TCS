...
export default function Configs() {
    ...
    const activateChangeUsername = () => {
        setChangeUsernameActive(!changeUsernameActive);
    }
    ...
    return (
        ...
        <div className="flex items-center justify-center min-h-screen dark:bg-gray-900">
            <div className="w-full max-w-4xl mx-4 sm:mx-8 xl:mx-auto">
                <div className="grid grid-cols-8 pt-3 sm:grid-cols-10"></div>
                <div className="col-span-8 overflow-hidden rounded-xl sm:bg-gray-50 sm:px-8 sm:shadow dark:bg-gray-800">
                    <div className="pt-4">
                        <p className="font- text-slate-600 dark:text-gray-300">Realize alterações importantes em sua conta.</p>
                    </div>
                    <div className="pt-4">
                        <h1 className="py-2 text-2xl font-semibold dark:text-white">Tema</h1>
                        <DarkModeToggle />
                    </div>
                    <hr className="mt-4 mb-8 border-gray-300 dark:border-gray-700" />
                    <p className="py-2 text-xl font-semibold dark:text-white">Endereço de E-mail</p>
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-gray-600 dark:text-gray-300">Seu E-mail é <strong>{user.email}</strong></p>
                    </div>
                    <hr className="mt-4 mb-8 border-gray-300 dark:border-gray-700" />
                    <p className="py-2 text-xl font-semibold dark:text-white">Nome de usuário</p>
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        {changeUsernameActive ? (
                            <input
                                ...
                                type="text"
                                className="border-black dark:bg-gray-700 dark:text-white"
                            />
                        ) : (
                            <p className="text-gray-600 dark:text-gray-300">Seu nome de usuário é <strong>{user.username}</strong></p>
                        )}
                        {changeUsernameActive ? (
                    ...
                    <hr className="mt-4 mb-8 border-gray-300 dark:border-gray-700" />
                    <p className="py-2 text-xl font-semibold dark:text-white">Senha</p>
                    <p className="text-gray-600 dark:text-gray-300">Sua senha atual é <strong>************</strong></p>
                    <Link to="/new-password">
                        <button className="inline-flex text-sm font-semibold text-blue-600 underline decoration-2">Alterar</button>
                    </Link>
                    <hr className="mt-4 mb-8 border-gray-300 dark:border-gray-700" />
                    <div className="mb-10">
                        <p className="py-2 text-xl font-semibold dark:text-white">Deletar Conta</p>
                        <p className="inline-flex items-center rounded-full bg-rose-100 px-4 py-1 text-rose-600 dark:bg-rose-900 dark:text-rose-300">
                            <svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                            </svg>
                            Prossiga com cautela
                        </p>
                        <p className="mt-2">Tenha Certeza para tomar esta decisão.</p> {isLoadingDelete && <Loading />}
                        {isAccountDelete && <p style={{ color: 'green' }}>Conta deletada com sucesso!</p>}
                        <button onClick={handleDeleteAccount} className="ml-auto text-sm font-semibold text-rose-600 underline decoration-2">Continuar com minha decisão</button>
                    </div>
                </div>
            </div>
        </div>
