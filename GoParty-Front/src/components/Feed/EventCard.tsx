...
const EventCard: React.FC<EventoCardProps> = ({ evento, userId, toggleComentarios, selectedEventoId }) => {
    ...
    return (
        <div key={evento.id} className="mt-14 max-w-xl mx-auto rounded overflow-hidden shadow-lg dark:shadow-lg dark:bg-gray-800">
            ...
            <div className="px-6 py-4 dark:bg-gray-700">
                ...
                <hr className="my-5 border-gray-300 dark:border-gray-700 lg:my-5" />
                <div className="mt-4 flex items-center">
                    <button
                        onClick={() => handleCurtir(evento.id, curtido)}
                        className={`flex mr-2 text-gray-700 text-sm mr-3 hover:text-indigo-500 ${loading ? 'cursor-not-allowed' : ''} dark:text-gray-300`}
                        disabled={loading}
                    >
                    ...
                    <button onClick={() => toggleComentarios(evento.id)} className="flex mr-2 text-gray-700 text-sm mr-8 hover:text-indigo-500 dark:text-gray-300">
