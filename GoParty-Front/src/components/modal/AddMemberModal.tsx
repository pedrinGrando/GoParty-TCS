...
export const AddMemberModal: React.FC<ModalProps> = ({ mostrarModal, onClose, formId }) => {
    ...
    const buscarUsuarios = async (search: string): Promise<UsuarioDTO[]> => {
        setIsLoading(true);
        try {
            const response = await fetch(`http://localhost:8081/v1/usuarios/ativos-estudantes?search=${encodeURIComponent(search)}`);
            if (!response.ok) {
